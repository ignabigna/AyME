...
ceros_PID = roots([K_d, K_p, K_i]);

%===============================================================
%=====================(VERSION FINAL INTEGRADA) ================
%====== GRÁFICOS DE POLOS (VERSIÓN 3 Y 4 CON POLO RESIDUAL) ====
%===============================================================

% 1. Cálculo del Polo Residual (s_res)
s_residual = -R_sREF / L_d; 

% --- Figura 3: Nominal (SIN lazo de corriente, CON Polo Residual) ---
figure(3);
hold on; grid on;

% Lazo Abierto (Negro, cruces)
plot(real([s1_Nmin, s2_Nmin, s3_Nmin]), imag([s1_Nmin, s2_Nmin, s3_Nmin]), ...
    'kx', 'MarkerSize', 12, 'LineWidth', 2, 'DisplayName', 'Lazo Abierto Nominal');

% Lazo Cerrado (Azul, círculos)
plot(real([s1, s2, s3]), imag([s1, s2, s3]), ...
    'bo', 'MarkerSize', 10, 'LineWidth', 2, 'DisplayName', 'Lazo Cerrado');

% Polo Residual (Negro, cruz extra)
plot(s_residual, 0, 'kx', 'MarkerSize', 12, 'LineWidth', 2, 'DisplayName', 'Polo Residual i_{ds}');

title('Mapa de Polos - Condición Nominal + Polo Residual');
...
% --- Figura 4: Nominal (CON lazo de corriente, CON Polo Residual) ---
figure(4);
hold on; grid on;
...
title('Mapa de Polos - Condición Nominal + Lazo Corriente + Polo Residual');
xlabel('Eje Real (\sigma)'); ylabel('Eje Imaginario (j\omega)');
legend('Location', 'best');
xline(0, 'k-', 'LineWidth', 1); yline(0, 'k-', 'LineWidth', 1);
hold off;
