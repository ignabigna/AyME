% =========================================================================
% SCRIPT DE ANÁLISIS DE TRANSITORIOS POR PERTURBACIÓN DE CARGA (Tl)
% =========================================================================
...
%% EVENTO 1: Primer impacto de carga Tl
t_in_1  = 0.3; % MODIFICAR: Tiempo de inicio del evento 1
t_fin_1 = 0.5; % MODIFICAR: Tiempo de fin del evento 1

idx1 = (tout >= t_in_1) & (tout <= t_fin_1);
analizar_y_mostrar('PERTURBACIÓN 1', tout(idx1), omega(idx1), i_q(idx1));

%% EVENTO 2: Segundo impacto de carga Tl
t_in_2  = 0.5; % MODIFICAR: Tiempo de inicio del evento 2
t_fin_2 = 0.7; % MODIFICAR: Tiempo de fin del evento 2

idx2 = (tout >= t_in_2) & (tout <= t_fin_2);
analizar_y_mostrar('PERTURBACIÓN 2', tout(idx2), omega(idx2), i_q(idx2));

%% EVENTO 3: Tercer impacto de carga Tl
t_in_3  = 0.9; % MODIFICAR: Tiempo de inicio del evento 3
t_fin_3 = 1.1; % MODIFICAR: Tiempo de fin del evento 3

idx3 = (tout >= t_in_3) & (tout <= t_fin_3);
analizar_y_mostrar('PERTURBACIÓN 3', tout(idx3), omega(idx3), i_q(idx3));

%% EVENTO 4: Cuarto impacto de carga Tl
t_in_4  = 1.3; % MODIFICAR: Tiempo de inicio del evento 4
t_fin_4 = 1.5; % MODIFICAR: Tiempo de fin del evento 4

idx4 = (tout >= t_in_4) & (tout <= t_fin_4);
analizar_y_mostrar('PERTURBACIÓN 4', tout(idx4), omega(idx4), i_q(idx4));

%% EVENTO 5: Quinto impacto de carga Tl
t_in_5  = 1.5; % MODIFICAR: Tiempo de inicio del evento 5
t_fin_5 = 1.7; % MODIFICAR: Tiempo de fin del evento 5

idx5 = (tout >= t_in_5) & (tout <= t_fin_5);
analizar_y_mostrar('PERTURBACIÓN 5', tout(idx5), omega(idx5), i_q(idx5));

%% EVENTO 6: Sexto impacto de carga Tl
t_in_6  = 1.9; % MODIFICAR: Tiempo de inicio del evento 6
t_fin_6 = 2.0; % MODIFICAR: Tiempo de fin del evento 6

idx6 = (tout >= t_in_6) & (tout <= t_fin_6);
analizar_y_mostrar('PERTURBACIÓN 6', tout(idx6), omega(idx6), i_q(idx6));


% =========================================================================
% FUNCIONES AUXILIARES (Intactas según el último acuerdo)
% =========================================================================
...
function analizar_y_mostrar(nombre_evento, t, w, iq)
    ...
    fprintf('  Tiempo de caída/recuperación(10-90%%): %.4f s\n', w_tr);
